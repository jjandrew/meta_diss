...
from searches.ga.fixing.fix import fixing


def fix(path: List[Tuple[int, int, int]], model: Dict[int, Hub]):
    """
    ...
    # Fix model for large chunks
    fixing(over_res_hubs=list(over_res_sur), under_res_hubs=list(
